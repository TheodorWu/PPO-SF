...
class GAE:
    ...
    def generalized_advantage_estimation(self, q_values, rewards, masks):
        advantage_estimates = torch.zeros(self.horizon, device=self.device)
        ...
        normalized_estimates = (advantage_estimates - torch.mean(advantage_estimates)) / (torch.std(advantage_estimates) + 1e-10)
        return advantage_estimates, normalized_estimates
